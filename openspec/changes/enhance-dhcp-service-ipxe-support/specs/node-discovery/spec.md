# node-discovery Specification Delta

## Purpose
增强 DHCP 服务器能力，添加完整的 TFTP 引导选项支持、IP 地址池管理和可选的 ProxyDHCP 模式。
...
- `node-installation`: 依赖 Option 66/67 正确设置以实现 iPXE 引导
- `rest-api`: 可能需要新增 IP 池管理 API（未来版本）
