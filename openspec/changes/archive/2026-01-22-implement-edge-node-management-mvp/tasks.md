...
- [x] **5.3 实现指令发布**
  ...
  - 错误处理和日志记录
  - **注**: MVP 阶段不实现指令发布（仅单向接收状态）

## Phase 6: RESTful API

- [x] **6.1 实现 Gin 路由和基础 Handler**
...
- [x] **6.3 实现节点注册和更新 API**
  ...
  - MAC 地址格式验证
  - 实现 `UpdateNode()` - PUT `/api/v1/nodes/:mac`
  - 处理 409 冲突
  - 编写集成测试

- [x] **6.4 实现安装触发 API**
  - 实现 `UpdateNode()` with `action: install` - PUT `/api/v1/nodes/:mac`
  - 验证节点状态（must be discovered）
  - 更新状态为 installing
  - 编写集成测试

- [x] **6.5 实现 iPXE 和 preseed 端点**
  - 实现 `GetBootScript()` - GET `/boot/:mac/boot.ipxe`
