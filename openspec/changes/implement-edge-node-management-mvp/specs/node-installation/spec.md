...
- `node-discovery`: 节点发现和 MAC 地址规范
- `node-state-management`: 节点状态转换（仅 discovered→installing→installed）
- `mqtt-reporting`: MQTT 状态接收（Agent 上报）
- `rest-api`: API 端点实现
